      ...
      <div className="max-w-6xl mx-auto px-6 relative z-10">
        <div className="text-center mb-16">
          ...
          <p className="mt-6 text-base leading-relaxed text-muted font-body max-w-xl mx-auto">
            Kies de manier die het beste bij u past. Of u nu een snelle vraag
            heeft of een compleet project wilt bespreken, wij staan voor u
            klaar.
          </p>
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-8 max-w-4xl mx-auto">
          {options.map((opt, i) => (
            <div
              key={i}
              className={`rounded-2xl p-8 border transition-all duration-300 hover:-translate-y-2 hover:shadow-xl flex flex-col h-full animate-[fade-in-up_0.6s_ease-out_both] ${
                opt.primary
                  ? "border-dark shadow-xl shadow-dark/10 bg-white"
            ...
            >
              <div
                className={`w-14 h-14 rounded-2xl flex items-center justify-center mb-6 ${
                  opt.primary ? "bg-dark" : opt.color.split(" ")[0]
                }`}
              >
                <Icon
                  name={opt.icon}
              ...
              <button
                onClick={opt.action}
                className={`w-full py-4 rounded-xl font-bold font-body text-sm transition-all duration-300 ${
                  opt.primary
                    ? "bg-brand text-white hover:bg-brand-dark hover:shadow-lg hover:shadow-brand/20"
                    : "bg-transparent border border-border/80 text-dark hover:bg-dark hover:text-white hover:border-dark"
                }`}
              >
                {opt.cta}
              </button>
